...
SELECT 
	(EXTRACT(YEAR FROM begin_date):: int / 10) * 10 AS decade,
	SUM(cpi_cost_full_amount) AS "total cpi adjusted cost per decade"
FROM us_natural_disaster_cost
GROUP BY decade
ORDER BY decade; 
...
SELECT DISTINCT disaster,
	AVG(deaths) AS "average death per disaster"
FROM us_natural_disaster_cost
GROUP BY disaster;
...
SELECT DISTINCT disaster,
	MAX(deaths) AS "deadliest disaster type"
FROM us_natural_disaster_cost
GROUP BY 1
ORDER BY 2 DESC;
